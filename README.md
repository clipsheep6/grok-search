...
- **并行搜索** - 复杂查询自动分解为多路并行搜索，提升 2-3 倍速度
...
```bash
# 独立多主题 - 自动并行搜索
grok-search "分别介绍 Rust、Go、Python 的特点"

# 对比查询 - 混合并行策略
grok-search "对比 Claude Sonnet 4.5 和 GPT-4o 的能力"

# 多维度分析 - 并行搜索
grok-search "全面分析 Rust：性能、生态、学习曲线、应用场景"
```
...
**并行搜索不是由 grok-search skill 自动执行的。** 这是一个**供主 agent 参考的策略指南**，主 agent 需要手动分解查询并启动多个 subagent 来实现并行搜索。

### 实现方式

当主 agent 检测到符合并行搜索模式的查询时，应该：

1. **分解查询**为 2-5 个独立的子查询
2. **并行启动多个 subagent**（在单个消息中发送多个 Task 调用）
3. **每个 subagent 调用 grok-search skill** 执行一个子查询
4. **收集所有结果**后进行综合分析
...
| 查询类型 | 触发模式 | 执行策略 | 加速比 |
|---------|---------|---------|--------|
| **独立多主题** | "分别介绍 A、B、C" | 纯并行 | 2.5x |
| **多维度分析** | "全面分析 X：维度1、维度2..." | 纯并行 | 2.7x |
| **对比查询** | "对比 A 和 B" / "A vs B" | 混合并行 | 2.7x |
| **递进查询** | "先...然后..." | 串行 | 1x |

### 混合并行策略（对比查询）

对于对比类查询，系统采用混合策略以最小化信息丢失：

```
查询："对比 Claude Sonnet 4.5 和 GPT-4o 的能力"

并行分解：
├─ Subagent 1: "Claude Sonnet 4.5 vs GPT-4o 对比评测" ← 保留原始对比
├─ Subagent 2: "Claude Sonnet 4.5 核心能力"          ← 细化 A
├─ Subagent 3: "GPT-4o 核心能力"                    ← 细化 B
└─ Subagent 4: "用户反馈和实际应用对比"             ← 补充维度

综合策略：
1. 优先使用直接对比结果（Subagent 1 & 4）
2. 用细化结果补充深度（Subagent 2 & 3）
3. 交叉验证一致性
```
...
- **最大并行数**：5 个 subagent（Claude Code 限制）
- **最小并行数**：2 个子查询
- **成本**：每个 subagent 独立消耗 API tokens
- **适用场景**：仅对符合触发模式的查询自动启用

详细策略说明请参考 `SKILL.md` 中的 "Parallel Search Strategy" 章节。
